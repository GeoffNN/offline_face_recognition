...
laplacian_param_regularization = gamma_g; %regularization to add to the laplacian (\gamma_g)
...
elseif strcmp(laplacian_normalization, 'sym')
    sD = inv(sqrtm(D));
    L = eye(length(W_q)) - sD*W_q*sD;
elseif strcmp(laplacian_normalization, 'rw')
    L = eye(length(W_q)) - inv(D)*W_q;
else
    error('unkown normalization mode')
end
Q = L + laplacian_param_regularization * eye(length(L));
...
f(l_idx) = y;
f(u_idx) = inv(Q(u_idx, u_idx))*(W_q(u_idx, l_idx)*f(l_idx)');
...
new_label_confidence = f(centroids_to_nodes_map == t);

new_label = sign(new_label_confidence);
