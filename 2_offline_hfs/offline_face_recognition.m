...
for i = 0:9
    for j = 1:10
        ...
        %gray_face = cv.boxFilter(gray_face);
        gray_face = cv.GaussianBlur(gray_face);
        %gray_face = cv.bilinearFilter(gray_face);
...
laplacian_param.regularization = 6; %regularization to add to the laplacian (\gamma_g)
...
c_u = .01
      ...
      labels = soft_hfs(X, Y_masked, c_l, c_u, graph_param, laplacian_param);
